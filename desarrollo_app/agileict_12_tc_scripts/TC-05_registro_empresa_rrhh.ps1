# TC-05 - Registro de empresa cliente y responsable RRHH
...
$adminToken = Login "admin@agileict.local" "demo1234"

$company = @{ nombre="Empresa TC05 $Uniq"; cif="B$Uniq"; sector="Tecnologia" }
$respCompany = Invoke-Api -Method "POST" -Path "/auth/companies" -Body $company -Token $adminToken
if ($respCompany.Status -ne 201) {
    Write-Host "[FAIL] TC-05 Crear empresa -> HTTP $($respCompany.Status)"
    Write-Host $respCompany.Body
    exit 1
}
$companyId = ($respCompany.Body | ConvertFrom-Json).id

$rrhh = @{
    responsableNombre="RRHH"; responsableApellidos="TC05"; responsableEmail="rrhh.$Uniq@test.local"; password="demo1234";
    empresaClienteId=$companyId
}
$respRrhh = Invoke-Api -Method "POST" -Path "/auth/register-rrhh" -Body $rrhh
Assert-Status "TC-05 Registro RRHH vinculado a empresa" $respRrhh @(201)
