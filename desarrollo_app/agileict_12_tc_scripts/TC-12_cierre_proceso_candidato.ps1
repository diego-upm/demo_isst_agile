# TC-12 - Cierre de proceso con candidato seleccionado
...
# Este TC es un flujo de aceptacion/integracion. El controlador de procesos actual expone GET, POST y DELETE.
# No se aprecia endpoint REST especifico para cerrar proceso con candidato seleccionado.
# Se comprueba como minimo que RRHH puede consultar procesos; el cierre debe validarse manualmente o con endpoint especifico si se implementa.
$rrhhToken = Login "rrhh@agileict.local" "demo1234"
$resp = Invoke-Api -Method "GET" -Path "/procesos" -Token $rrhhToken
if ($resp.Status -eq 200) {
    Write-Host "[SKIP] TC-12 requiere endpoint de cierre/contratacion. Consulta de procesos OK, cierre pendiente de endpoint/UI."
    exit 0
}
Assert-Status "TC-12 Consulta previa de procesos" $resp @(200)
